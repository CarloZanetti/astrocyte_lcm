...
protein_dict <- fread(file.path(objects_dir, "clean_spec_raw.csv")) %>%
  select(Protein = PG.ProteinGroups, Gene = PG.Genes) %>%
  distinct()
...
## Correlate gene levels to Gfap

```{r}
final_clean_meta <- read.csv(file.path(objects_dir, "msstats_annotation.csv"))
...
#calculate R between every gene's abundance and GFAP. 
...
# View the top 20
#seems to just be garbage genes
head(pn_gfap_cors, 20)
```
