...
generate_swanky_volcano <- function(msstats_results, dict, comparison_label, logfc_cutoff = 0.58, graphs_dir = "results") {
  ...
  export_data <- volcano_data %>%
    select(Protein, Gene, Description, log2FC, pvalue, adj.pvalue, Status) %>%
    arrange(adj.pvalue)
...
floor_check <- res_df %>%
  ...
  select(Protein, log2FC, pvalue, adj.pvalue, SE, DF) %>%
