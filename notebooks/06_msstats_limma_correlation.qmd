...
millie_combined <- bind_rows(
  ...
  mutate(
    Status_limma = case_when(
      adj.pvalue_limma < 0.05 & log2FC_limma > 0 ~ "Upregulated",
      adj.pvalue_limma < 0.05 & log2FC_limma < -0 ~ "Downregulated",
      TRUE ~ "Not Significant"
    )
  ) %>%
...
plot_data <- comparison_df %>%
  filter(Label == "PlaqueNear_vs_Control") %>% # Matches your Millie DF assignment
  mutate(
    Category = case_when(
      Status_msstats != "Not Significant" & Status_limma != "Not Significant" ~ "Both",
      Status_msstats != "Not Significant" & Status_limma == "Not Significant" ~ "MSstats Only",
      Status_msstats == "Not Significant" & Status_limma != "Not Significant" ~ "limma Only",
      TRUE ~ "Not Significant"
    ),
    LabelGene = ifelse(Category != "Not Significant", Gene, NA)
  )
```
...
# 2. Summarize the findings per comparison
...
# View the proteins
print(limma_exclusive_sig)
```

```{r}
check_cutoffs <- comparison_df %>%
  filter(Label == "PlaqueNear_vs_PlaqueFar") %>%
